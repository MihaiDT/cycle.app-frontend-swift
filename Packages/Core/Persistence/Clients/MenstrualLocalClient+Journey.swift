...
extension MenstrualLocalClient {
    static func liveJourneyData() -> @Sendable () async throws -> JourneyData {
        return {
            ...
            // Use `journeyStartDate` as a defensive lower bound when
            // it's set, but never let it hide logged cycles: any cycle
            // already on disk is, by definition, part of the user's
            // journey. If `journeyStartDate` is somehow nil (legacy
            // data, reset mid-flow), fall back to showing everything
            // rather than an empty journey.
