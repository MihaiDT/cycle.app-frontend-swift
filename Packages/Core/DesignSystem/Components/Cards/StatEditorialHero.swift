    ...
    public var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            HStack(alignment: .firstTextBaseline, spacing: 8) {
                Text(value)
                    .font(.raleway("Bold", size: 72, relativeTo: .largeTitle))
                    .tracking(-2.0)
            ...
            // Soft warm glow rendered behind the number without
            // forcing the VStack to grow. `.background` sizes to the
            // hero text only; the Circle then overflows visually via
            // its 260pt frame + blur without contributing to layout.
