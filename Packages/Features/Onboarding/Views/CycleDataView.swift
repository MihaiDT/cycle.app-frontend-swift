...
public enum SymptomType: String, CaseIterable, Identifiable, Sendable {
    ...
    static var physicalSymptoms: [SymptomType] {
        [
            .cramping, .headache, .backPain, .bloating, .breastTenderness, .nausea, .acne, .dizziness, .hotFlashes,
            .jointPain,
        ]
    }

    static var digestiveSymptoms: [SymptomType] {
        [.constipation, .diarrhea, .appetiteChanges, .cravings]
    }
                    ...
                    VStack(spacing: 12) {
                        ...
                        HStack(spacing: 8) {
                            Circle()
                                .fill(
                                    tutorialStep == .selectStart
                                        ? DesignColors.accent : DesignColors.accent.opacity(0.3)
                                )
                                .frame(width: 8, height: 8)

                            Rectangle()
                                .fill(
                                    tutorialStep == .selectEnd ? DesignColors.accent : DesignColors.accent.opacity(0.3)
                                )
            ...
            HStack(spacing: 24) {
                Button(action: {
                    ...
                    Image(systemName: "minus.circle.fill")
                        .font(.system(size: 44))
                        .foregroundColor(
                            value > range.lowerBound ? DesignColors.accent : DesignColors.text.opacity(0.2)
                        )
                }
                ...
                Button(action: {
                    ...
                    Image(systemName: "plus.circle.fill")
                        .font(.system(size: 44))
                        .foregroundColor(
                            value < range.upperBound ? DesignColors.accent : DesignColors.text.opacity(0.2)
                        )
                }
        ...
        Button(action: action) {
            ...
            .background {
                RoundedRectangle(cornerRadius: 16)
                    .fill(
                        isSelected ? DesignColors.accent.opacity(0.1) : Color(UIColor.secondarySystemGroupedBackground)
                    )
                    .overlay {
                        RoundedRectangle(cornerRadius: 16)
                            .strokeBorder(isSelected ? DesignColors.accent : Color.clear, lineWidth: 2)
                    }
            }
        }
        .buttonStyle(.plain)
...
private struct InlineSymptomsSelector: View {
    ...
    private let categories: [(String, [SymptomType])] = [
        (
            "Physical",
            [
                .cramping, .headache, .backPain, .bloating, .breastTenderness, .nausea, .dizziness, .hotFlashes,
                .jointPain,
            ]
        ),
        ("Digestive", [.constipation, .diarrhea, .appetiteChanges, .cravings]),
        ("Mood", [.moodSwings, .irritable, .anxious, .sad, .emotional, .sensitive]),
        ("Energy", [.lowEnergy, .tired, .normalEnergy, .highEnergy]),
        ("Sleep", [.peacefulSleep, .difficultyFallingAsleep, .restlessSleep, .insomnia]),
        ("Skin", [.acne, .oilySkin, .drySkin, .skinBreakouts]),
    ]
            ...
            FlowLayout(spacing: 10) {
                ...
                Button(action: {
                    ...
                    Text("None")
                        ...
                        .foregroundColor(!usesContraception ? DesignColors.accent : .primary.opacity(0.7))
                        .padding(.horizontal, 16)
                        .padding(.vertical, 12)
                        .background {
                            Capsule()
                                .strokeBorder(
                                    !usesContraception ? DesignColors.accent : Color.gray.opacity(0.3),
                                    lineWidth: !usesContraception ? 1.5 : 1
                                )
                                .background(
                                    Capsule().fill(!usesContraception ? DesignColors.accent.opacity(0.1) : Color.clear)
                                )
                        }
                }
                .buttonStyle(.plain)
                ...
                ForEach(ContraceptionType.allCases) { type in
                    Button(action: {
                        ...
                        Text(type.displayName)
                            .font(.custom("Raleway-Medium", size: 14))
                            .foregroundColor(contraceptionType == type ? DesignColors.accent : .primary.opacity(0.7))
                            .padding(.horizontal, 16)
                            .padding(.vertical, 12)
                            .background {
                                Capsule()
                                    .strokeBorder(
                                        contraceptionType == type ? DesignColors.accent : Color.gray.opacity(0.3),
                                        lineWidth: contraceptionType == type ? 1.5 : 1
                                    )
                                    .background(
                                        Capsule().fill(
                                            contraceptionType == type ? DesignColors.accent.opacity(0.1) : Color.clear
                                        )
                                    )
                            }
                    }
        ...
        Button(action: action) {
            HStack(spacing: 6) {
                if let customIcon = symptom.customIcon,
                    let uiImage = UIImage(named: customIcon, in: .main, with: nil)
                {
            ...
            .background {
                Capsule()
                    .strokeBorder(
                        isSelected ? DesignColors.accent : DesignColors.text.opacity(0.2),
                        lineWidth: isSelected ? 1.5 : 1
                    )
                    .background(
                        Capsule()
                            .fill(isSelected ? DesignColors.accent.opacity(0.15) : Color.white.opacity(0.5))
                    )
            }
        }
        .buttonStyle(.plain)
